...
void processVideo(char* videoFilename) {
	...
	while( (char)keyboard != 'q' && (char)keyboard != 27 && capture.get(CV_CAP_PROP_POS_FRAMES) < capture.get(CV_CAP_PROP_FRAME_COUNT) ){
		...
		cvtColor(reference_frame, reference_frame_gray, CV_BGR2GRAY);
		...
		subtract(reference_frame, reference_frame_gray_move, reference_frame_gray);
			...
			if(contourArea(contours[i]) > 200) {
				...
				if(delay > 80) {
					...
					if(abs(x - rect.x) + abs(y - rect.y) < 200) {
						...
						drawContours(reference_frame_gray, contours, i, color_blue, 2, LINE_AA);
		...
		// top-left = new objects
		reference_frame.copyTo(combine(Range(0, frame.size().height), Range(0, frame.size().width)));
		// bottom-left = moving objects
		reference_frame_gray_move.copyTo(combine(Range(frame.size().height, frame.size().height*2), Range(0, frame.size().width)));
		// bottom-right = new but non-moving objects with its contours
		reference_frame_gray.copyTo(combine(Range(frame.size().height, frame.size().height*2), Range(frame.size().width, frame.size().width*2)));
		// top-right = highlighted static objects on original frame
